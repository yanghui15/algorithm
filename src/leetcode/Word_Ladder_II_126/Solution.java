...
public class Solution {
	...
	public void dfs(Node end , List<List<String>> result , List<String> cur , Node start , int count , HashSet<Node> set){
		cur.add(end.str);
		if(cur.size() > count) return;
		if(start == end){
			if(cur.size() > count) return;
		...
		for(Node i : end.last){
			if(set.contains(i))
				continue;
			set.add(i);
			dfs(i , result , cur , start , count , set);
...
*/
		for(int i = 0 ; i < idx ; i ++){
			char cur[] = node[i].str.toCharArray();
			for(int j = 0 ; j < cur.length ; j ++){
				char temp = cur[j];
				for(int k = 0 ; k < 26 ; k ++){
					if(cur[j] - 'a' == k) continue;
					cur[j] = (char)('a' + k);
					if(wordList.contains(new String(cur))){
						node[i].next.add(node[map.get(new String(cur))]);
					}
				}
				cur[j] = temp;
			}
		...
		HashSet<Node> dict = new HashSet<Node>();
		dict.add(node[n+1]);

		dfs(node[n+1] , result , cur , node[n] , count , dict);
